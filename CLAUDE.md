...
**Turnstile (self-hosted embed).** Cloudflare Turnstile cannot run on the packaged app's `tauri://localhost` origin, so the auth screen renders it in an iframe served from a loopback HTTP server owned by the Rust side ([embed_server.rs](src-tauri/src/embed_server.rs), ephemeral port, `GET /turnstile` only). The page itself is [public/turnstile-embed.html](public/turnstile-embed.html) — compiled into the binary via `include_str!` and also served by Vite for browser-mode dev — and posts `{source: "qipaoshui-turnstile", event: ready|token|expired|error}` back to [Turnstile.tsx](src/components/Turnstile.tsx). The message shape is a contract between those two files. Operational requirement: `localhost` must be in the Turnstile sitekey's allowed domains (Cloudflare dashboard); the server's siteverify does not check token hostname, so no qipaoshui server changes are involved.
